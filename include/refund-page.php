...
<body>
    <div class="container">
        <div class="header clearfix">
            <h3 class="text-muted">HOÀN TIỀN</h3>
        </div>
        ...
        <div style="width: 100% ;border-bottom: 2px solid black;padding-bottom: 20px">
            <form action="<?php echo (home_url('/wp-admin/admin.php?page=qr-pay3')); ?>" id="frmCreateOrder" method="post">
                <div class="form-group">
                    <label>Mã GD thanh toán cần hoàn (vnp_TxnRef):</label>
                    <input class="form-control" data-val="true" name="TxnRef" type="text" value="<?php if (isset($_GET['TxnRef']) && !empty($_GET['TxnRef'])) {
                                                                                                        echo $_GET['TxnRef'];
                                                                                                    } ?>" />
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" max="100000000" min="1" name="Amount" type="number" value="<?php if (isset($_GET['amount']) && !empty($_GET['amount'])) {
                                                                                                                echo $_GET['amount'];
                                                                                                            } ?>" />
                </div>
                <div class="form-group">
                    <label>Thời gian khởi tạo GD thanh toán (vnp_TransactionDate):</label>
                    <input class="form-control" data-val="true" name="TransactionDate" type="text" placeholder="yyyyMMddHHmmss" value="<?php if (isset($_GET['TransactionDate']) && !empty($_GET['TransactionDate'])) {
                                                                                                                                            echo str_replace(['-', ':', ' '], '', $_GET['TransactionDate']);
                                                                                                                                        } ?>" />
                </div>
                <div class="form-group">
        ...
        require_once(plugin_dir_path(__FILE__) . '../vnpay/config.php');
        require_once(plugin_dir_path(__FILE__) . '../db/db_payments.php');
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($ispTxn !== null && isset($ispTxn["vnp_ResponseCode"])
            ) { // Kiểm tra dữ liệu hợp lệ
                if ($ispTxn["vnp_ResponseCode"] == "00") {
                    update_status_payments((int)$ispTxn["vnp_TxnRef"],2);
                    echo "Hoàn tiền thành công";
                } else {
                    echo "Hoàn tiền thất bại: " . htmlspecialchars($ispTxn["vnp_ResponseCode"]);
                }
            }
            // echo "
            // <div>
